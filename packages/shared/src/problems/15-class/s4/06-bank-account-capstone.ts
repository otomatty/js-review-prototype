...
- \`constructor(initialBalance = 0)\`: \`initialBalance\` が **0 未満なら例外** (\`Error\`) を投げる。 そうでなければ \`#balance\` を初期化する。
- \`deposit(amount)\`: \`amount > 0\` のとき \`#balance\` に加算する。 \`amount <= 0\` のときは **何もしない** (例外は投げない)。
- \`withdraw(amount)\`: \`amount > 0\` かつ \`amount <= #balance\` のとき残高を引いて \`true\` を返す。 そうでなければ **残高は変えずに \`false\`** を返す。
- \`getBalance()\`: 現在の \`#balance\` を返す (これが残高を外から読む唯一の経路)。
