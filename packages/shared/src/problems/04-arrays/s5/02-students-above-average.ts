...
- 推奨フロー:
  ...
  2. \`scores\` の合計を for ループまたは \`reduce\` で求め、 平均 \`avg\` を計算する
...
- AST で **\`map\` の使用** と **\`filter\` の使用** と **\`sort\` の使用** を必須にしています (S5 でのチェーン書きを推奨)。
- ループで書いても OK ですが、 設計演習として **\`map → filter → sort → map\`** の **パイプライン構造** を意識して書くと S5 らしさが出ます。
`,
...
`,
  ...
  hints: [
    "まず平均 avg を求めます: const avg = scores.reduce((a, b) => a + b, 0) / scores.length;  (空配列のときに 0 除算しないよう早期 return しておくと安全)。",
